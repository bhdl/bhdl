...
;; I actually don't want to hold onto this, because I don't want to restart the server in each notebook session because it is slow until the TCP port is being listening, and the placement engine warm up is also slow. I'll just monitor the 8082 port.
...
;; if no process this kill will throw error
;; (shell "kill -9 `lsof -i :8082 | awk '{print $2}' | tail -n 1`")
...
;; (define (status-placement-engine)
;;   "Close placement engine."
;;   (if (the-placement-process)
;;       (begin
;;         (displayln ((list-ref (the-placement-process) 4) 'status)))
;;       (displayln "No active placement engine.")))
