...
(define (start-placement-engine [port 8082])
  ...
  ;; TODO how to launch the process? Using costodian or process?
  ...
  (when (not (the-placement-process))
...
(define (wait-for-8082)
  (for ([i (in-naturals)]
     ...
     (display (~a "The TCP port is still " (get-8082-processid) " , wainting for 1 sec .. process status: "))
     (status-placement-engine)
     (flush-output)
     (sleep 1))
